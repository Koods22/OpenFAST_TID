...
function Matlab2SD(HDPar,TemplateFile,OutputFilename,hdrLines)
...
while true
    ...
    if ~isComment && ~isempty(label)          
        ...
        elseif strcmpi(value,'"X"') %we've reached the user-defined spring force table (and we think it's a string value so it's in quotes)
            if ~isfield(HDPar,'NKInpSt')
                disp( 'WARNING: the member joints table not found in the HD data structure.' );
                printTable = true;
            else
                frmt = ' %4i %11.5f %11.5f %11.5f %6i %12i';
                WriteFASTTable(line, fidIN, fidOUT, HDPar.NKInpSt, newline, frmt);
                continue; %let's continue reading the template file            
            end    
