...
if fid == -1
    error(['HD file, ' HD_file ', could not be opened for reading. Check if the file exists or is locked.'])
end
...
while true %loop until discovering Outlist or end of file, than break
    ...
    elseif ~isComment
        ...
        elseif strcmpi(value,'"X"') %we've reached the User-defined spring forces table (and we think it's a string value so it's in quotes)
            NForces = GetFASTPar(DataOut,'NKInpSt');        
            [DataOut.Forces, DataOut.ForcesHdr] = ParseFASTTable(line, fid, NForces);
        ...
        elseif strcmpi(value,'"JointID"') %we've reached the member joints table (and we think it's a string value so it's in quotes)
            ...
            [DataOut.Forces, DataOut.ForcesHdr] = ParseFASTTable(line, fid, NJoints);
            continue; %let's continue reading the file
